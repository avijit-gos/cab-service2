...
class DriverController {
    ...
    async createNewDriver(req, res, next) {
        try {
          ...
          return res.status(201).json({message: "New driver added",statusCode: 201, data: driverData})
        } catch (error) {
            next(error)
        }
    }
    ...
    async updateDriverDetails(req, res, next) {
        try {
           ...
           const updatedDriverData = await Driver.findByIdAndUpdate(id, req.body, {new: true});
           return res.status(200).json({message: "Driver data successfully updated", statusCode: 200, data: updatedDriverData})
        }
        catch(error) {
            next(error)
        }
    ...
    async deleteDriverData(req, res, next) {
        try {
            ...
            return res.status(200).json({message: "Driver data successfully deleted",statusCode: 200, data: deletedData})
        } catch (error) {
            next(error)
        }
    }
